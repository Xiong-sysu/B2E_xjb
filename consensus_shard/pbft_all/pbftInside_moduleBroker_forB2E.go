	...
	if rbhm.pbftNode.NodeID == rbhm.pbftNode.view {
		...
		for _, tx := range block.Body {
			...
			if tx.IsAllocatedRecipent || tx.IsAllocatedSender {
				...
				if tx.IsBroker2Tx {
					...
					if rsid == ssid {
						...
						relayer_1_2_txs = append(relayer_1_2_txs, tx) // 这里剩下的不是bat、broker、普通itx，那就是relay 和 has broker
						if tx.HasBroker {
							h++
						}
					}
			...
			row := []string{
				...
				strconv.FormatInt(tx.Relay2_time.UnixMilli(), 10), // 毫秒
		...
		rbhm.pbftNode.writeCSVline([]string{strconv.Itoa(int(block.Header.Number)), strconv.Itoa(len(rbhm.pbftNode.CurChain.Txpool.TxQueue)), strconv.Itoa(rbhm.pbftNode.CurChain.Txpool.GetRelayPoolSize()), strconv.Itoa(bim.BlockBodyLength), strconv.Itoa(len(txExcuted)),
