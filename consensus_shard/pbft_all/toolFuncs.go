...
func (p *PbftConsensusNode) writeCSVline(str []string) {
	...
	if err != nil && os.IsNotExist(err) {
		...
		title := []string{"blockHeight", "txpool size", "BlockBodyLength", "txExcuted", "broker1Txs", " broker2Txs", "AllocatedTxs", "BAT_byte_Size", "block_byte_Size", "BAT_byte_ratio", "Relay1TxNum", "Relay2TxNum", "innerTxs", "relayer_1_2_txs"}
		w.Write(title)
		w.Flush()
		w.Write(str)
		w.Flush()
